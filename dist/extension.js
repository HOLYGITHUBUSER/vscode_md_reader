...
function taskListPlugin(md2) {
  md2.core.ruler.after("inline", "task-lists", (state) => {
    for (const token of state.tokens) {
      if (token.type === "inline" && token.children) {
        for (const child of token.children) {
          if (child.type === "text") {
            child.content = child.content.replace(/\[x\]/gi, "\u2611").replace(/\[ \]/g, "\u2610");
          }
        }
      }
    }
...
function toggleView(current) {
  return current === "preview" /* Preview */ ? "source" /* Source */ : "preview" /* Preview */;
}
function getStatusBarText(mode) {
  return mode === "preview" /* Preview */ ? "$(eye) \u9884\u89C8" : "$(code) \u539F\u59CB";
}
var StatusBarController = class {
  constructor(onToggle) {
    this.onToggle = onToggle;
    this.currentMode = "preview" /* Preview */;
  ...
  updateDisplay() {
    ...
    this.item.tooltip = this.currentMode === "preview" /* Preview */ ? "\u70B9\u51FB\u5207\u6362\u5230\u539F\u59CB\u5185\u5BB9" : "\u70B9\u51FB\u5207\u6362\u5230\u9884\u89C8";
  }
  toggle() {
...
function activate(context) {
  ...
  const openPreviewCmd = vscode3.commands.registerCommand("md-reader.openPreview", () => {
    const doc = vscode3.window.activeTextEditor?.document;
    if (doc) {
  ...
  const toggleViewCmd = vscode3.commands.registerCommand("md-reader.toggleView", () => {
    statusBarController.toggle();
  });
  const changeDocSub = vscode3.workspace.onDidChangeTextDocument((e) => {
